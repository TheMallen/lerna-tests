...
import ReactCSSTransitionGroup = require('react-addons-transition-group');
...
export default class TransitionGroup extends React.Component<Props, never> {
  ...
  render() {
    ...
    const wrapperComponent =
      component || (React.Children.count(children) <= 1 ? FirstChild : 'div');

    const wrappedChildren = elementChildren(children).map((child, index) => {
      ...
      return <TransitionChild key={index} {...transitionChildProps} />;
    });
